...
func start_stems(p_stems: Array, p_crossfade_time: float) -> void:
	...
	for stem in p_stems:
		...
		# https://github.com/godotengine/godot/issues/100696
...
func toggle_stem(p_name: String, p_enabled: bool, p_fade_time: float) -> void:
	if not _stem_table.has(p_name):
		push_warning(
			(
				"Resonate - Cannot toggle the stem [%s] on music track [%s] from bank [%s] as it does not exist."
