...
func play(
	p_bank_label: String,
	p_track_name: String,
	p_crossfade_time: float = 5.0,
	p_auto_loop: bool = false
) -> bool:
	if not has_loaded:
		push_error(
			(
	...
	if not _music_table.has(p_bank_label):
		push_error(
			(
				"Resonate - Tried to play the music track [%s] from an unknown bank [%s]."
				% [p_track_name, p_bank_label]
			)
		)
		return false
	...
	if not _music_table[p_bank_label]["tracks"].has(p_track_name):
		push_error(
			(
				"Resonate - Tried to play an unknown music track [%s] from the bank [%s]."
				% [p_track_name, p_bank_label]
			)
		)
		return false
	...
	if stems.size() == 0:
		push_error(
			(
				"Resonate - The music track [%s] on bank [%s] has no stems, you'll need to add one at minimum."
				% [p_track_name, p_bank_label]
			)
		)
		return false
	...
	var player = StemmedMusicStreamPlayer.create(
		p_bank_label, p_track_name, bus, bank.mode, _volume, p_auto_loop
	)
...
func stop(p_fade_time: float = 5.0) -> void:
	if not _is_playing_music():
		push_warning(
			"Resonate - Cannot stop the music track as there is no music currently playing."
		)
		return
...
func set_stem_volume(p_name: String, p_volume: float) -> void:
	if not _is_playing_music():
		push_warning(
			(
				"Resonate - Cannot set the volume of stem [%s] as there is no music currently playing."
				% p_name
			)
		)
		return
...
func get_stem_details(p_name: String) -> Variant:
	if not _is_playing_music():
		push_warning(
			(
				"Resonate - Cannot get the details for stem [%s] as there is no music currently playing."
				% p_name
			)
		)
		return
...
func stop_on_exit(
	p_base: Node, p_bank_label: String, p_track_name: String, p_fade_time: float = 5.0
) -> void:
	p_base.tree_exiting.connect(
		_on_music_player_exiting.bind(p_bank_label, p_track_name, p_fade_time)
	)
...
func _add_bank(p_bank: MusicBank) -> void:
	if _music_table.has(p_bank.label):
		_music_table[p_bank.label]["ref_count"] = _music_table[p_bank.label]["ref_count"] + 1
...
func _get_bus(p_bank_bus: String, p_track_bus: String) -> String:
	...
	return ProjectSettings.get_setting(
		_settings.MUSIC_BANK_BUS_SETTING_NAME, _settings.MUSIC_BANK_BUS_SETTING_DEFAULT
	)
...
func _set_stem(p_name: String, p_enabled: bool, p_fade_time: float) -> void:
	if not _is_playing_music():
		push_warning(
			(
				"Resonate - Cannot toggle the stem [%s] as there is no music currently playing."
				% p_name
			)
		)
		return
...
func _on_music_player_exiting(
	p_bank_label: String, p_track_name: String, p_fade_time: float
) -> void:
...
func _on_auto_loop_completed(
	p_bank_label: String, p_track_name: String, p_crossfade_time: float
) -> void:
