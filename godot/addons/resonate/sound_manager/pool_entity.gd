...
static func configure(
	...
	var max_polyphony = ProjectSettings.get_setting(
		_settings.MAX_POLYPHONY_SETTING_NAME, _settings.MAX_POLYPHONY_SETTING_DEFAULT
	)
...
static func trigger(p_base, p_varied: bool, p_pitch: float, p_volume: float) -> void:
	if p_base.streams.size() == 0:
		push_warning(
			(
				"Resonate - The player [%s] does not contain any streams, ensure you're using the SoundManager to instance it correctly."
				% p_base.name
			)
		)
		return
...
static func release(p_base, p_finish_playing: bool) -> void:
	...
	if p_finish_playing and has_loops:
		push_warning(
			(
				"Resonate - The player [%s] has looping streams and therefore will never release itself back to the pool (as playback continues indefinitely). It will be forced to stop."
				% p_base.name
			)
		)
		p_base.stop()
