...
func _ready() -> void:
	_initialise_pool(
		ProjectSettings.get_setting(
			_settings.POOL_1D_SIZE_SETTING_NAME, _settings.POOL_1D_SIZE_SETTING_DEFAULT
		),
		_create_player_1d
	)
	...
	_initialise_pool(
		ProjectSettings.get_setting(
			_settings.POOL_2D_SIZE_SETTING_NAME, _settings.POOL_2D_SIZE_SETTING_DEFAULT
		),
		_create_player_2d
	)
	...
	_initialise_pool(
		ProjectSettings.get_setting(
			_settings.POOL_3D_SIZE_SETTING_NAME, _settings.POOL_3D_SIZE_SETTING_DEFAULT
		),
		_create_player_3d
	)
...
## [/codeblock]
func quick_instance(
	p_instance, p_factory: Callable, p_base: Node = null, p_finish_playing: bool = false
) -> Variant:
...
func play_at_position(
	p_bank_label: String, p_event_name: String, p_position, p_bus: String = ""
) -> void:
...
## exclusive use of it until it is told to [method PooledAudioStreamPlayer.release] or is automatically
## released when registered with [method SoundManager.release_on_exit].
func instance_on_node(
	p_bank_label: String, p_event_name: String, p_node, p_bus: String = ""
) -> Variant:
...
## [b]Note:[/b] This method will mark the reserved player as polyphonic (able to play
## multiple event variations simultaneously.)
func instance_at_position_poly(
	p_bank_label: String, p_event_name: String, p_position, p_bus: String = ""
) -> Variant:
...
func _add_bank(p_bank: SoundBank) -> void:
	if _event_table.has(p_bank.label):
		_event_table[p_bank.label]["ref_count"] = _event_table[p_bank.label]["ref_count"] + 1
...
func _get_bus(p_bank_bus: String, p_event_bus: String) -> String:
	...
	return ProjectSettings.get_setting(
		_settings.SOUND_BANK_BUS_SETTING_NAME, _settings.SOUND_BANK_BUS_SETTING_DEFAULT
	)


func _instance_manual(
	p_bank_label: String,
	p_event_name: String,
	p_reserved: bool = false,
	p_bus: String = "",
	p_poly: bool = false,
	p_attachment = null
) -> Variant:
	...
	if not _event_table.has(p_bank_label):
		push_error(
			(
				"Resonate - Tried to instance the event [%s] from an unknown bank [%s]."
				% [p_event_name, p_bank_label]
			)
		)
		return _get_null_player(p_attachment)
	...
	if not _event_table[p_bank_label]["events"].has(p_event_name):
		push_error(
			(
				"Resonate - Tried to instance an unknown event [%s] from the bank [%s]."
				% [p_event_name, p_bank_label]
			)
		)
		return _get_null_player(p_attachment)
	...
	if event.streams.size() == 0:
		push_error(
			(
				"Resonate - The event [%s] on bank [%s] has no streams, you'll need to add one at minimum."
				% [p_event_name, p_bank_label]
			)
		)
		return _get_null_player(p_attachment)
	...
	if player == null:
		push_warning(
			(
				"Resonate - The event [%s] on bank [%s] can't be instanced; no pooled players available."
				% [p_event_name, p_bank_label]
			)
		)
		return _get_null_player(p_attachment)
