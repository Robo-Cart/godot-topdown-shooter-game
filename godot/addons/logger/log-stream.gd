...
static func _static_init() -> void:
	_ensure_setting_exists(
		settings.LOG_MESSAGE_FORMAT_KEY, settings.LOG_MESSAGE_FORMAT_DEFAULT_VALUE
	)
	_ensure_setting_exists(
		settings.USE_UTC_TIME_FORMAT_KEY, settings.USE_UTC_TIME_FORMAT_DEFAULT_VALUE
	)
	_ensure_setting_exists(settings.BREAK_ON_ERROR_KEY, settings.BREAK_ON_ERROR_DEFAULT_VALUE)
	_ensure_setting_exists(
		settings.PRINT_TREE_ON_ERROR_KEY, settings.PRINT_TREE_ON_ERROR_DEFAULT_VALUE
	)
...
func _init(
	...
	crash_behavior: Callable = default_crash_behavior
):
	_log_name = log_name
	...
	if err_code != OK:
		call_thread_safe(
			...
			(
				...
				else "." + " Error string: " + error_string(err_code)
			),
			other_values_to_be_printed,
			LogLevel.FATAL if fatal else LogLevel.ERROR
		)
...
func err_cond_not_equal(
	...
	if (arg1 is Color && arg2 is Color && !arg1.is_equal_approx(arg2)) || arg1 != arg2:
		call_thread_safe(
			"_internal_log",
			str(arg1) + " != " + str(arg2) + ", not allowed. " + message,
			other_values_to_be_printed,
			LogLevel.FATAL if fatal else LogLevel.ERROR
		)
...
func _get_format_data(msg: String, lvl: LogLevel) -> Dictionary:
	...
	var format_data := {"log_name": _log_name, "message": msg, "level": LogLevel.keys()[lvl]}
		...
		TYPE_DICTIONARY:
			...
			for k in values:
				if typeof(values[k]) == TYPE_OBJECT && values[k] != null:
					msg += '"{k}":{v},'.format(
						{"k": k, "v": JSON.stringify(JsonData.to_dict(values[k], false))}
					)
				else:
