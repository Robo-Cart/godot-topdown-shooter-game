...
func _on_create_file_pressed():
	var new_name: String = file_dialog.get_child(3, true).get_child(3, true).get_child(1, true).text
	if new_name == "":
...
func _on_export_csv_pressed():
	...
	ResourceTablesExportFormatCsv.export_to_file(
		editor_view.rows, exported_cols, import_data.edited_path, import_data
	)
	await get_tree().process_frame
	editor_view.refresh()
	close()
...
func _on_export_space_toggled(button_pressed: bool):
	import_data.delimeter = (
		import_data.delimeter[0] if !button_pressed else import_data.delimeter + " "
	)
