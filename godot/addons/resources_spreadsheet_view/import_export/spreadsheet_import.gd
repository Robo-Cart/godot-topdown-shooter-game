...
func string_to_property(string: String, col_index: int):
	match prop_types[col_index]:
		...
		PropType.BOOL:
			...
			if string == enum_format[2].to_lower():
				return true
			if string == enum_format[3].to_lower():
				return false
...
func create_enum_for_prop(col_index) -> String:
	...
	for k in uniques[col_index]:
		result += ("\t" + k + " = " + str(uniques[col_index][k]) + ",\r\n")  # Enum Entry  # Value
	result += "\tMAX,\r\n}\r\n\r\n"
	return result
...
func strings_to_resource(strings: Array):
	...
	if prop_used_as_filename != "":
		new_res.resource_path = (
			edited_path.get_basename() + "/" + new_res.get(prop_used_as_filename) + ".tres"
		)
