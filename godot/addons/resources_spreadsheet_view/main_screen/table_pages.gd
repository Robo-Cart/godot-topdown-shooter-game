...
func update_page_count(array: Array) -> int:
	...
	node_editor_view_root.last_row = min(
		node_editor_view_root.first_row + rows_per_page, array.size()
	)
	return page_count
...
func _on_grid_updated():
	...
	var sort_type: int = node_editor_view_root.column_types[node_editor_view_root.columns.find(
		sort_property
	)]
	var property_values := []
