...
@onready
var node_property_editors: VBoxContainer = $"../HeaderContentSplit/MarginContainer/FooterContentSplit/Footer/PropertyEditors"
@onready
var scrollbar: ScrollContainer = $"../HeaderContentSplit/MarginContainer/FooterContentSplit/Panel/Scroll"
...
func _ready():
	...
	for x in cell_editor_classes:
		...
		all_cell_editors[all_cell_editors.size() - 1].hint_strings_array = (
			editor_view.column_hint_strings
		)

	get_parent().editor_interface.get_inspector().property_edited.connect(
	...
	if ProjectSettings.get_setting(
		editor_view.TablesPluginSettingsClass.PREFIX + "fold_docks", false
	):
...
func _draw():
	...
	for i in edited_cells.size():
		...
		var caret_rect := TextEditingUtilsClass.get_caret_rect(
			edited_cells_text[i], edit_cursor_positions[i], font, font_size, label_padding_left, 2.0
		)
...
func select_cells_to(cell: Vector2i):
	...
	for i in range(row_start, row_end, edge_shift):
		...
		if cur_cell not in edited_cells:
			...
			var cur_cell_value = editor_view.io.get_value(
				editor_view.rows[cur_cell.y], editor_view.columns[cur_cell.x]
			)
		...
		if (
			cur_cell_node == null
			or !cur_cell_node.visible
			or cur_cell_node.mouse_filter == MOUSE_FILTER_IGNORE
		):
...
func _on_inspector_property_edited(property: String):
	...
	if (
		inspector_resource
		!= editor_view.editor_plugin.get_editor_interface().get_inspector().get_edited_object()
	):
		return
