...
const TablesPluginEditorViewClass := preload(
	"res://addons/resources_spreadsheet_view/editor_view.gd"
)
const TablesPluginSelectionManagerClass := preload(
	"res://addons/resources_spreadsheet_view/main_screen/selection_manager.gd"
)
...
func _key_specific_action(event: InputEvent):
	...
	if TextEditingUtilsClass.multi_move_caret(
		caret_move_offset,
	...
	if (
		!selection.column_editors[column].is_text()
		or editor_view.columns[column] == "resource_path"
	):
		return
	...
	# ERASING
		...
		editor_view.set_edited_cells_values_text(
			TextEditingUtilsClass.multi_erase_left(
		...
		editor_view.set_edited_cells_values_text(
			TextEditingUtilsClass.multi_erase_right(
				selection.edited_cells_text, selection.edit_cursor_positions, ctrl_pressed
			)
		)
		get_viewport().set_input_as_handled()
