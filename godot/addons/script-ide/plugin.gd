...
func _enter_tree() -> void:
	...
	if scripts_item_list != null:
		...
		script_filter_txt = find_or_null(
			scripts_item_list.get_parent().find_children("*", "LineEdit", true, false)
		)
		script_filter_txt.gui_input.connect(navigate_on_list.bind(scripts_item_list, select_script))

	# Make tab container visible
	scripts_tab_container = find_or_null(
	...
	if split_container != null:
		...
		outline_filter_txt = find_or_null(
			outline_container.find_children("*", "LineEdit", true, false), 1
		)
...
func open_outline_popup():
	...
	outline_popup.popup_hide.connect(
		on_outline_popup_hidden.bind(outline_initially_closed, old_text, button_flags)
	)

	if outline_popup.get_parent() != null:
		outline_popup.get_parent().remove_child(outline_popup)
	outline_popup.popup_exclusive_on_parent(
		EditorInterface.get_script_editor(), get_center_editor_rect()
	)
...
func on_outline_popup_hidden(
	outline_initially_closed: bool, old_text: String, button_flags: Array[bool]
):
...
func open_scripts_popup():
	...
	scripts_popup.popup_exclusive_on_parent(
		EditorInterface.get_script_editor(), get_center_editor_rect()
	)
...
func sync_settings():
	...
	for setting: String in changed_settings:
		...
		if setting == OUTLINE_POSITION_RIGHT:
			...
			var new_hide_private_members: bool = get_setting(
				HIDE_PRIVATE_MEMBERS, hide_private_members
			)
			...
			var new_script_list_visible: bool = get_setting(
				SCRIPT_LIST_VISIBLE, is_script_list_visible
			)
...
func update_outline_cache():
	...
	if base_script != null:
		for_each_script_member(
			base_script, func(array: Array[String], item: String): array.erase(item)
		)
...
func add_to_outline(
	items: Array[String], icon: ImageTexture, type: String, modifier: StringName = &""
):
	add_to_outline_ext(items, func(str: String): return icon, type, modifier)


func add_to_outline_ext(
	items: Array[String], icon_callable: Callable, type: String, modifier: StringName = &""
):
	...
	for item: String in items:
		if text.is_empty() || text.is_subsequence_ofn(item):
			...
			var dict: Dictionary = {&"type": type, &"modifier": modifier}
...
func sync_tab_with_script_list():
	...
	if (
		selected_tab != -1
		&& scripts_item_list.item_count > 0
		&& !scripts_item_list.is_selected(selected_tab)
	):
...
func simulate_item_clicked(tab_idx: int, mouse_idx: int):
	...
	scripts_item_list.item_clicked.emit(
		tab_idx, scripts_item_list.get_local_mouse_position(), mouse_idx
	)
...
func is_sorted() -> bool:
	return get_editor_settings().get_setting(
